    ...
    switch (error) {
      case "file-invalid-type":
        const acceptedTypes = Object.values(limits.accept ?? {})
          .flat()
          .join(", ");
        return `Sadece ${acceptedTypes} dosya türlerine izin var.`;
      case "file-too-large":
        const maxMb = limits.maxSize
          ? (limits.maxSize / (1024 * 1024)).toFixed(2)
          : "infinite?";
        return `Maksimum dosya boyutu: ${maxMb}MB`;
      case "file-too-small":
        const roundedMinSize = limits.minSize
          ? (limits.minSize / (1024 * 1024)).toFixed(2)
          : "negative?";
        return `min dsoya boyutu ${roundedMinSize}MB`;
      case "too-many-files":
        return `Maksimum ${limits.maxFiles} adet dosya ekleyebilirsiniz.`;
    }
  });
...
// There is no obvious bug in this file, so "fix this page" may refer to missing `export default`. 
// If this should be the default export for the Dropzone component, add this line:
